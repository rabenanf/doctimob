...
export const NewRequestScreen = ({ navigation }: Props): JSX.Element => {
  ...
  const [familyMemberDropdownVisible, setFamilyMemberDropdownVisible] =
    useState(false);
  const [familyMembers, setFamilyMembers] = useState<any[]>([]);
  const [familyMembersList, setFamilyMembersList] = useState<any[]>([]);
  const [familyMember, setFamilyMember] = useState<string | undefined>(
    undefined
  );
  ...
  const handleFamilyMemberSelect = (member: string) => {
    let selected = familyMembersList.filter((item) => {
      return item.first_name + " " + item.last_name == member;
    });
    console.log("####", member, "####", selected);
    setFamilyMemberId(selected[0].id);
    setFamilyMemberId(member);
  };
  const [specialtyDropdownVisible, setSpecialtyDropdownVisible] =
    useState(false);
  ...
  const createRequestAction = async () => {
    ...
    if (familyMember) data.family_mamber_id = familyMemberId;

    if (specialty) (data.specialty_id = spec), console.log("--data----", data);
  ...
  useEffect(() => {
    let names = specialties.map((item) => {
      return item.name;
    });
      ...
      <ScrollView contentContainerStyle={styles.detailContainer}>
        ...
        <View style={styles.medicalInfo}>
          <Text style={styles.sectionTitle}>
            {t("NewRequest.medicalInfoTitle").toUpperCase()}
          </Text>
          ...
          <View style={styles.shareHistoryContainer}>
            <CustomToogle
              label={t("NewRequest.shareHistory")}
              handleEnabled={function (value: boolean): void {}}
              defaultState={false}
            />
          </View>
          ...
          <View style={styles.methodContainer}>
            ...
            >
              <Text style={!hasInsurance ? styles.textSelected : styles.text}>
                {t("NewRequest.No")}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
        ...
        <View style={styles.dateTime}>
          <Text style={styles.sectionTitle}>
            {t("NewRequest.preferredDateTitle").toUpperCase()}
          </Text>
          <View
            style={{ flexDirection: "row", justifyContent: "space-between" }}
          >
            <Text style={{ fontSize: 12, fontWeight: 400 }}>
              {t("NewRequest.chooseDay")}
            </Text>
        ...
        <View style={styles.consultation}>
          <Text style={styles.sectionTitle}>
            {t("NewRequest.consultationTitle").toUpperCase()}
          </Text>
          <View style={styles.methodContainer}>
            <TouchableOpacity
              style={[
                consultationMethod == TypeConsultation.HOMEVISIT
                  ? styles.methodSelected
                  : styles.method,
            ...
            <TouchableOpacity
              style={[
                consultationMethod == TypeConsultation.ONLINE
                  ? styles.methodSelected
                  : styles.method,
                ,
                { justifyContent: "space-between", paddingHorizontal: 16 },
          ...
          <View style={styles.methodContainer}>
            <TouchableOpacity
              style={
                preferredlanguage == Language.BOTH
                  ? styles.methodSelected
                  : styles.method
              }
            ...
            <TouchableOpacity
              style={
                preferredlanguage == Language.VIET
                  ? styles.methodSelected
                  : styles.method
              }
        ...
        <View style={styles.payment}>
          <Text style={styles.sectionTitle}>
            {t("NewRequest.paymentTitle").toUpperCase()}
          </Text>
          <View style={styles.methodContainer}>
            <TouchableOpacity
              style={
            ...
            >
              {paymentMethod == PaymentMethod.CASH ? (
                ...
                <CashBlackIcon height={24} width={24} />
              )}
              <Text
                style={
                  paymentMethod == PaymentMethod.CASH
